...
function refetch (A, name, delay = 100) {
    ...
    return {
        saga: function* () {
            ...
            const { id } = yield effects.select(state => state[name].current);
            ...
            if (!id) {
                ...
                yield effects.put(A.readOne({ payload: id }));
            }
        },
...
const makeRequest = (instance, method, path, ...rest) => {
    ...
    request[reduxSaga.CANCEL] = () => source.cancel('request was cancelled by sagas');
...
var AxiosWrapper = (options = {}) => {
    ...
    // Default options
    options = {
        baseURL: ((typeof window !== 'undefined' && window.location) || {}).hostname || 'http://axios.baseURL.not.set',
        timeout: 5000,
        ...options,
        headers: {
            ...headers,
...
function* makeCall(instance, method, path, ...args) {
    ...
    try {
        const { data } = yield effects.call(...callArgs);

        yield effects.put(success(data));

        donePayload = { data };
    }
    catch (error) {

        yield effects.put(fail(error));

        if (error.response) {

            const { status } = error.response;
            yield effects.put({ type: `sagaApi/${status}`, payload: error.response });
        }
    ...
    if (done) {
        ...
        yield effects.put(done(donePayload));
    }
}
